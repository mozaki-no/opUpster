# リポ直下で
git switch -c feat/boot-cli-logging-ci
...
chmod +x modernize.sh
./modernize.sh

# 2) 実行確認（CLIヘルプ/バージョン）
./mvnw spring-boot:run -- -h || true
./mvnw spring-boot:run -- -v || true

# 3) コミット & プッシュ
git add -A
git commit -m "feat: Spring Boot(17) CLI化 + structured logging + tests + CI"
git push -u origin feat/boot-cli-logging-ci
